...
// TestNullCache 验证缓存穿透防御(空值缓存,分散在对等节点的 getLocally):
// getter 返回 not-found 时塞占位符,后续同 key 命中占位符挡住 getter(不再回源),仍返回 ErrKeyNotFound。
func TestNullCache(t *testing.T) {
	loadCounts := make(map[string]int)
	gee := NewGroup("scores-null", 2<<10, "lru", GetterFunc(
		func(key string) ([]byte, error) {
			loadCounts[key]++
			return nil, ErrKeyNotFound // not-found:getLocally 据此塞占位符
		}))
	defer gee.Stop()

	// ① 首次 Get:回源 1 次,塞占位符,返回 ErrKeyNotFound
	...
	// ② 第二次:命中占位符,不再回源(防穿透关键),仍返回 ErrKeyNotFound
	if _, err := gee.Get("nope"); err == nil {
